(ns moralis.main
  "Kotodama WASM entrypoint for the Moralis clean-room actor (L4) — Clojure port.
  ...
  facts (`moralis.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "moralis")
(def tier "L4")
...
(def entity-specs
  [{:entity "Block"       :plural "blocks"       :id-prefix "moralis_blo"
    :fields [:number :hash :chain :minedAt]      :required [:number :hash]
    :coerce {:number :int}                 :refs {}}
   {:entity "Transaction" :plural "transactions" :id-prefix "moralis_tra"
    :fields [:hash :fromAddr :toAddr :value :status] :required [:hash :fromAddr]
    :coerce {:value :float}                :refs {}}
   {:entity "Contract"    :plural "contracts"    :id-prefix "moralis_con"
    :fields [:address :chain :abiRef]      :required [:address :chain]
    :coerce {}                             :refs {}}
   {:entity "Account"     :plural "accounts"     :id-prefix "moralis_acc"
    :fields [:address :chain :balance :nonce]    :required [:address :chain]
    :coerce {:balance :float :nonce :int}  :refs {}}
   {:entity "Event"       :plural "events"       :id-prefix "moralis_eve"
    :fields [:contractAddr :name :blockNumber :dataRef] :required [:contractAddr :name]
    :coerce {:blockNumber :int}            :refs {}}
   {:entity "Token"       :plural "tokens"       :id-prefix "moralis_tok"
    :fields [:contractAddr :symbol :decimals :standard] :required [:contractAddr :symbol]
    :coerce {:decimals :int}               :refs {}}])
...
(defn emit-facts
  "EAVT facts for one record: {\"moralis.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "moralis-compat" :tier tier :entities entities} 200])
...
(defn start! [] :moralis-compat/ready)
